...
function LastSentences({ sentences }: LastSentencesProps) {
  return (
    <div
      className={clsx(
        'mx-auto mt-6 flex max-w-3xl flex-col gap-3 px-3 pb-12 font-noto duration-500 md:px-1',
      ...
      {sentences.map((s) => (
        ...
        >
          <div className="flex gap-1 sm:items-baseline sm:justify-between sm:gap-4">
            <div className="order-3 text-sm text-gray-400 sm:order-none group-hover:md:text-gray-900">
              {s.value}
            </div>
            ...
            <ReactTimeAgo
              className="order-1 cursor-default pr-1 pl-1 text-center text-(length:--text-xs) leading-5 text-gray-light sm:order-none sm:text-gray-400 group-hover:md:text-blue-royal"
